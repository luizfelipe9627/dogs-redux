...
// Cria um componente chamado Loading.
const Loading = () => {
  const [step, setStep] = React.useState(1); // Criado um estado chama step e uma função atualizadora chamada setStep. O valor inicial de step é 0.

  // Criado um efeito colateral que é executado após a renderização do componente. O efeito colateral é responsável por atualizar o estado step a cada 500 milissegundos.
  React.useEffect(() => {
    // Criado uma função chamada updateStep que é responsável por atualizar o estado step.
    function updateStep() {
      // Está atualizando o estado step passando uma função como parâmetro. A função recebe o valor do estado step como parâmetro e retorna um valor. O valor retornado é o valor do estado step + 1.
      setStep((step) => {
        // Se o valor do estado step for menor que 3, retorna o valor do estado step + 1. Se não, retorna 1.
        if (step < 3) {
          return step + 1;
        } else {
          return 1;
    ...
    const interval = setInterval(updateStep, 300); // Executa a função updateStep a cada 500 milissegundos, armazenando a referência do intervalo em uma variável chamada interval.

    // Executa o retorno quando o componente for desmontado(sair da tela).
    return () => clearInterval(interval); // Retorna uma função que limpa o intervalo armazenado na variável interval.
  }, []);

  // Criado uma função chamada displayStep que recebe um parâmetro chamado index, a função é responsável por exibir o elemento de acordo com o valor do estado step.
  function displayStep(index) {
    // Retorna um objeto com a propriedade display que recebe o valor block ou none.
    return {
      display: step === index ? "block" : "none", // Se o estado step for igual ao valor do parâmetro index inserido na função displayStep, aplica o display block fazendo com que o elemento seja exibido. Se não, aplica o display none fazendo com que o elemento não seja exibido.
    };
  }
  ...
  return (
    <div className={styles.wrapper}>
      <div className={styles.loading}>
        ...
        >
          {/* Chama a função displayStep responsável por exibir o elemento de acordo com o valor do estado step e passa o valor 0 como parâmetro. */}
          ...
          {/* Chama a função displayStep responsável por exibir o elemento de acordo com o valor do estado step e passa o valor 1 como parâmetro. */}
          ...
          {/* Chama a função displayStep responsável por exibir o elemento de acordo com o valor do estado step e passa o valor 2 como parâmetro. */}
          ...
          {/* Chama a função displayStep responsável por exibir o elemento de acordo com o valor do estado step e passa o valor 3 como parâmetro. */}
